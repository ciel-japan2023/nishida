...
for (let y=0; y<invRow; y++){
	for (let x=0; x<invRowQty; x++){
		invaders.push({
			...
			y: y * invW + (y + 2) * invDistance+30,//最初の位置
			flg: true
		});
...
// 横方向に移動する回数
const invMoveXCnt = 100;  //インベーダーの動き遅くなる
// 縦方向に移動する回数
const invMoveYCnt = 20;	//インベーダーの動き遅くなる
// インベーダーの横方向に進む距離
...
for (let y=0; y<8; y++){
	for (let x=0; x<11; x++){
		if (invLayout1[y][x] == 0){
			...
			invElm1 += x * invDotW + "px " + y * invDotH +"px red,";//色変更
		}
		if (invLayout2[y][x] == 0){
			invElm2 += x * invDotW + "px " + y * invDotH +"px black,";
		}
		else {
			invElm2 += x * invDotW + "px " + y * invDotH +"px red,";
		}
		if (invLayout3[y][x] == 0){
			invElm3 += x * invDotW + "px " + y * invDotH +"px black,";
		}
		else {
			invElm3 += x * invDotW + "px " + y * invDotH +"px red,";
		}
	}
}
...
$("#inv_bullet").css({
	"position": "absolute",
	"width": invDotW * 2 + "px",//球の幅
	"height": invDotH *2 + "px",//球の長さ invDoth*数
	"margin-left": invBullet.x +"px",
	"margin-top": invBullet.y +"px",
	"box-shadow": invDotW + "px " + invDotH +"px red" //球の色
});
...
function invaderAct(stepCnt){
	for (let i=0; i<invaders.length; i++){
		if (invaders[i]["flg"]){
			...
			// インベーダーレイアウト変更
			if (stepCnt % 10 <= 0.5){//数字が小さいとバンザイする感覚狭くなる
	...
	// 弾の処理
	invBullet.y += invDotH * 10//数字を変えると球を出す速度早くなる（インベーダ）
